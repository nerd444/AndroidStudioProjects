...
public class MainActivity extends AppCompatActivity {
    ...
    ImageView diceImg1;     //onCreate 함수안에 선언하면 그 안에서밖에 사용못해서 밖에 선언.
    ImageView diceImg2;

    MediaPlayer mp;

    // 5. 주사위 실제 이미지 (1~6)를 코드로 가져온다.
    // 6. 이미지를 int 로 가져온 이유는? 안드로이드에서 자동으로 우리가 넣은 이미지를, 정수로 바꿔서 관리합니다.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        diceImg2 = findViewById(R.id.imgDice2);    // 화면 맨아래 주사위 두개를 가져온것
       ...
       btnRoll.setOnClickListener(new View.OnClickListener() {
           ...
           public void onClick(View v) {
               // 3. 이 안에다 동작시키고 싶은 코드를 작성한다. i = info, e = error

               // 3-1.버튼이 눌렸을때 실행 순서
               Log.i("MyDiceApp", "btnRollDice is tapped!");
               // 3-2. 랜덤으로 한개 숫자 가져와서
               Random rand = new Random();
               int diceNumber = rand.nextInt(6);
               Log.i("MyDiceApp", ""+diceNumber); // firstDiceNumber는 숫자 문자열로 바꿔주는 게 앞에 ""+ 붙이는것
               // 3-3. 가져온 숫자에 맞는 주사위이미지를 셋팅한다.
               // diceNumber 는 0~5까지 나온다.
               // 따라서 diceNumber 에 해당하는숫자가, 이미지배열의 인덱스와 같다.
